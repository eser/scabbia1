<?php
				...
				define('SCABBIA_VERSION', '1.0.15410');
				define('INCLUDED', 'Scabbia 1.0.15410');
...
<?php
 class Framework { public static $includePaths = array(); public static $downloadUrls = array(); public static $development; public static $debug; public static $repository; public static function translatePath($uPath) { if(substr($uPath, 0, 6) == '{core}') { return QPATH_CORE . substr($uPath, 6); } if(substr($uPath, 0, 5) == '{app}') { return QPATH_APP . substr($uPath, 5); } return $uPath; } public static function load() { self::$development = Config::get('/options/development/@value', '0'); self::$debug = Config::get('/options/debug/@value', '0'); self::$repository = Config::get('/options/repository/@value', '0'); if(!COMPILED) { $tDownloads = Config::get('/downloadList', array()); foreach($tDownloads as &$tDownload) { self::$downloadUrls[$tDownload['@filename']] = $tDownload['@url']; } self::downloadFiles(); $tIncludes = Config::get('/includeList', array()); foreach($tIncludes as &$tInclude) { self::$includePaths[] = self::translatePath($tInclude['@path']); } self::includeFilesFromConfig(); } } public static function run() { if(OUTPUT_GZIP && !PHP_SAPI_CLI && Config::get('/options/gzip/@value', '1') != '0') { ob_start('ob_gzhandler'); } if(OUTPUT_MULTIBYTE) { ob_start('mb_output_handler'); } if(!COMPILED) { foreach(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS) as $tValue) { if($tValue['type'] == 'include') { $tIncluded = true; } } if(PHP_SAPI_CLI) { $tParameters = implode(' ', array_slice($GLOBALS['argv'], 1)); } else { $tParameters = $_SERVER['QUERY_STRING']; } if(!isset($tIncluded)) { if(self::$development) { if($tParameters == 'build') { self::build('index.php'); self::purgeTemp(); self::purgeCompiledTemplates(); echo 'build done.'; return; } } if(self::$repository) { if(substr($tParameters, 0, 4) == 'rep:') { $tFile = QPATH_CORE . substr($tParameters, 4); echo php_strip_whitespace($tFile); return; } } } } Events::invoke('run', array()); } public static function downloadFiles() { foreach(self::$downloadUrls as $tFilename => &$tUrl) { self::downloadFile($tFilename, $tUrl); } } public static function downloadFile($uFile, $uUrl) { $tFilePath = QPATH_APP . 'downloaded/' . $uFile; if(file_exists($tFilePath)) { return false; } $tHandle = fopen($tFilePath, 'w'); $tContent = file_get_contents($uUrl); fwrite($tHandle, $tContent); fclose($tHandle); return true; } private static function includeFilesFromConfig() { foreach(self::$includePaths as &$tPath) { foreach(glob($tPath, GLOB_MARK|GLOB_NOSORT) as $tFilename) { if(substr($tFilename, -1) == '/') { continue; } require($tFilename); } } } private static function printIncludeFilesFromConfig() { foreach(self::$includePaths as &$tPath) { self::printFiles(glob($tPath, GLOB_MARK|GLOB_NOSORT)); } } public static function printFiles($uArray) { foreach($uArray as &$tFilename) { if(substr($tFilename, -1) == '/') { continue; } echo php_strip_whitespace($tFilename); } } public static function build($uFilename) { ob_start(); echo '<', '?php
				ignore_user_abort();
				date_default_timezone_set(\'UTC\');
				setlocale(LC_ALL, \'en_US.UTF-8\');
				mb_internal_encoding(\'UTF-8\');
...
<?php
 class repository { private static $packageKey = null; private static $packages = array(); public static function extension_info() { return array( 'name' => 'repository', 'version' => '1.0.2', 'phpversion' => '5.1.0', 'fwversion' => '1.0', 'enabled' => true, 'autoevents' => false, 'depends' => array('string', 'http', 'database') ); } public static function extension_load() { if(COMPILED) { Events::register('run', Events::Callback('repository::run')); } foreach(Config::get('/repository/packageList', array()) as $tPackage) { self::$packages[$tPackage['@name']] = array(); foreach($tPackage['fileList'] as &$tFile) { self::$packages[$tPackage['@name']][] = Framework::translatePath($tFile['@path']); } } } public static function run() { $tCheckKey = Config::get('/repository/routing/@repositoryCheckKey', 'rep'); $tCheckValue = Config::get('/repository/routing/@repositoryCheckValue', ''); $tPackageKey = Config::get('/repository/routing/@repositoryPackageKey', 'rep'); if(array_key_exists($tCheckKey, $_GET)) { if(strlen($tCheckValue) == 0) { self::$packageKey = $_GET[$tPackageKey]; } else if($_GET[$tCheckKey] == $tCheckValue) { self::$packageKey = $_GET[$tPackageKey]; } } if(isset(self::$packageKey)) { if(array_key_exists(self::$packageKey, self::$packages)) { Framework::printFiles(self::$packages[self::$packageKey]); return false; } } } public static function getPackageKey() { return self::$packageKey; } } ?>
<?php
