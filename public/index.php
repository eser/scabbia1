<?php
				...
				define('SCABBIA_VERSION', '1.0.15406');
				define('INCLUDED', 'Scabbia 1.0.15406');
...
<?php
 ...
 class users extends Controller { public function unsubscribe() { $this->loadmodel('usersModel', 'users'); $tEmail = $this->httpGet(2, ''); if(empty($tEmail)) { return $this->error('user is empty'); } $tResult = $this->users->unsubscribe($tEmail); $tViewbag = array( 'title' => 'Done' ); if($tResult) { $tViewbag['message'] = 'Your e-mail is unsubscribed from mailing list. You won\'t get any notification mails from now on.'; } else { $tViewbag['message'] = 'Your e-mail has already been unsubscribed from mailing list.'; } $this->loadview('shared_error.cshtml', $tViewbag); } public function error($uMsg) { $tViewbag = array( 'title' => 'Error', 'message' => $uMsg ); $this->loadview('shared_error.cshtml', $tViewbag); } public function notfound() { return $this->error('404 not found!'); } } ?><?php
 class accountsModel extends Model { public function __construct($uController) { parent::__construct($uController); $this->db->setDatabaseName('dbconn'); } function checkLogin($uName, $uPassword) { $tPassword = database::get('dbconn', 'getLoginPassword')->queryScalar($uName); if(!is_null($tPassword) && $tPassword == md5($uPassword)) { return true; } return false; } } ?><?php
