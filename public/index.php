<?php
	...
	define('SCABBIA_VERSION', '1.0.15434');
	define('INCLUDED', 'Scabbia 1.0.15434');
...
?><?php
 if(!function_exists('fnmatch')) { function fnmatch($uPattern, $uString) { for($tBrackets = 0, $tPregPattern = '', $tCount = 0, $tLen = strlen($uPattern); $tCount < $tLen; $tCount++) { $tChar = $uPattern[$tCount]; if(strpbrk($tChar, '\\/-+^$=!.|(){}<>')) { $tPregPattern .= '\\' . $tChar; } else if(strpbrk($tChar, '?*')) { $tPregPattern .= '.' . $tChar; } else { $tPregPattern .= $tChar; if($tChar == '[') { $tBrackets++; } else if($tChar == ']') { if($tBrackets == 0) { return false; } $tBrackets--; } } } if($tBrackets != 0) { return false; } return preg_match('/' . $tPregPattern . '/i', $uString); } } if(!function_exists('glob2')) { function glob2($uPattern) { $tPath = pathinfo($uPattern, PATHINFO_DIRNAME); if(($tDir = opendir($tPath)) !== false) { $tGlob = array(); while(($tFile = readdir($tDir)) !== false) { if($tFile == '.' || $tFile == '..') { continue; } $tFile2 = $tPath . '/' . $tFile; if(fnmatch($uPattern, $tFile2)) { $tGlob[] = $tFile2; } } closedir($tDir); return $tGlob; } return false; } } ?><?php
...
<?php
 class Framework { public static $includePaths = array(); public static $downloadUrls = array(); public static $development; public static $debug; public static $siteroot; public static $directCall; public static function translatePath($uPath) { if(substr($uPath, 0, 6) == '{core}') { return QPATH_CORE . substr($uPath, 6); } if(substr($uPath, 0, 5) == '{app}') { return QPATH_APP . substr($uPath, 5); } return $uPath; } public static function load() { self::$development = (bool)Config::get('/options/development/@value', '0'); self::$debug = (bool)Config::get('/options/debug/@value', '0'); self::$siteroot = Config::get('/options/siteroot/@value', ''); self::$directCall = !COMPILED; if(strlen(self::$siteroot) <= 1) { $tLen = strlen($_SERVER['DOCUMENT_ROOT']); if(substr(QPATH_CORE, 0, $tLen) == $_SERVER['DOCUMENT_ROOT']) { self::$siteroot = strtr(substr(QPATH_CORE, $tLen), DIRECTORY_SEPARATOR, '/'); } } self::$siteroot = rtrim(self::$siteroot, '/'); Extensions::init(); if(!COMPILED) { $tDownloads = Config::get('/downloadList', array()); foreach($tDownloads as &$tDownload) { self::$downloadUrls[$tDownload['@filename']] = $tDownload['@url']; } self::downloadFiles(); $tIncludes = Config::get('/includeList', array()); foreach($tIncludes as &$tInclude) { self::$includePaths[] = self::translatePath($tInclude['@path']); } self::includeFilesFromConfig(); } } public static function output($uValue, $uSecond) { $tParms = array( 'content' => &$uValue ); Events::invoke('output', $tParms); if(OUTPUT_MULTIBYTE) { $tParms['content'] = mb_output_handler($tParms['content'], $uSecond); } if(OUTPUT_GZIP && !PHP_SAPI_CLI && Config::get('/options/gzip/@value', '1') != '0') { $tParms['content'] = ob_gzhandler($tParms['content'], $uSecond); } return $tParms['content']; } public static function run() { ob_start('Framework::output'); ob_implicit_flush(false); if(!COMPILED) { if(version_compare(PHP_VERSION, '5.3.6', '>=')) { $tBacktrace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS); } else { $tBacktrace = debug_backtrace(false); } foreach($tBacktrace as &$tValue) { if(isset($tValue['function']) && ($tValue['function'] == 'include' || $tValue['function'] == 'require')) { self::$directCall = false; } } if(PHP_SAPI_CLI) { $tParameters = array_slice($_SERVER['argv'], 1); } else { $tParameters = $_GET; } if(self::$directCall) { if(self::$development && count($tParameters) > 0) { if($tParameters[0] == 'build') { self::build('index.php', !(count($tParameters) >= 2 && $tParameters[1] == 'pseudo')); self::purgeFolder(QPATH_APP . 'temp'); self::purgeFolder(QPATH_APP . 'sessions'); echo 'build done.'; return; } } exit('why?'); } } } public static function downloadFiles() { foreach(self::$downloadUrls as $tFilename => &$tUrl) { self::downloadFile($tFilename, $tUrl); } } public static function downloadFile($uFile, $uUrl) { $tFilePath = QPATH_APP . 'downloaded/' . $uFile; if(file_exists($tFilePath)) { return false; } $tHandle = fopen($tFilePath, 'w'); $tContent = file_get_contents($uUrl); fwrite($tHandle, $tContent); fclose($tHandle); return true; } private static function includeFilesFromConfig() { foreach(self::$includePaths as &$tPath) { foreach(glob2($tPath) as $tFilename) { if(substr($tFilename, -1) == '/') { continue; } require($tFilename); } } } private static function printIncludeFilesFromConfig() { foreach(self::$includePaths as &$tPath) { self::printFiles(glob2($tPath)); } } public static function printFiles($uArray) { foreach($uArray as &$tFilename) { if(substr($tFilename, -1) == '/') { continue; } $tContent = php_strip_whitespace($tFilename); $tOpenTags = 0; foreach(token_get_all($tContent) as $tToken) { if($tToken[0] == T_OPEN_TAG || $tToken[0] == T_OPEN_TAG_WITH_ECHO) { $tOpenTags++; } else if($tToken[0] == T_CLOSE_TAG) { $tOpenTags--; } } echo $tContent; for(;$tOpenTags > 0;$tOpenTags--) { echo ' ?', '>'; } } } public static function build($uFilename, $uForce = true) { ob_start(); ob_implicit_flush(false); if(self::$development && !$uForce) { echo '<', '?php
...
<?php Config::set(array (
  ...
  '/access/maintenance/@page' => '{app}static/maintenance.htm',
  '/access/ipFilter/@page' => '{app}static/ipban.htm',
  '/access/ipFilterList' => 
  array (
    0 => 
    array (
  ...
  '/mvc/routing/@controllerUrlKey' => '1',
  '/mvc/routing/@actionUrlKey' => '2',
  '/mvc/view/@namePattern' => '{@controller}_{@action}_{@device}_{@language}{@extension}',
 ...
 if(Extensions::isSelected('access')) { class access { public static $mode = 0; public static $ipFilters = array(); public static function extension_info() { return array( 'name' => 'access', 'version' => '1.0.2', 'phpversion' => '5.1.0', 'phpdepends' => array(), 'fwversion' => '1.0', 'fwdepends' => array('http') ); } public static function extension_load() { Events::register('run', Events::Callback('access::run')); self::$mode = intval(Config::get('/access/maintenance/@mode', '0')); foreach(Config::get('/access/ipFilterList', array()) as $tIpFilterList) { if(preg_match('/^' . str_replace(array('.', '*', '?'), array('\\.', '[0-9]{1,3}', '[0-9]{1}'), $tIpFilterList['@pattern']) . '$/i', $_SERVER['REMOTE_ADDR'])) { if($tIpFilterList['@type'] == 'allow') { self::$ipFilters = array(); continue; } self::$ipFilters[] = $tIpFilterList['@pattern']; } } } public static function run() { if(self::$mode) { $tFile = Framework::translatePath(Config::get('/access/maintenance/@page')); include($tFile); return false; } if(count(self::$ipFilters) > 0) { $tFile = Framework::translatePath(Config::get('/access/ipFilter/@page')); include($tFile); return false; } } } } ?>
<?php
